...
showpackagestats(){
	...
	echo -e "${COL_BLUE}|$NO_COLOR    Archlinux Core Dump   $COL_GREEN($NAME $VERSION)$COL_BLUE |${NO_COLOR}"	
	echo -e "${COL_BLUE} -------------------------------------------${NO_COLOR}\n"	
	echo -e "\n${COL_BLUE}-----------------------------------------------${NO_COLOR}"	
	echo -e "${COL_GREEN}Total installed packages:  ${COL_YELLOW}`pacman -Q | wc -l`"	
	echo -e "${COL_GREEN}Explicitly installed packages:  ${NO_COLOR}${COL_YELLOW}`pacman -Qe | wc -l`"	
	echo -e "${COL_GREEN}Packages installed as dependencies to run other packages:  ${COL_YELLOW}`pacman -Qd | wc -l`"   
	local orphans=(`pacman -Qdt | awk '{print $1}' | sort`)
	if [  ${#orphans[@]} -gt 0 ]; then 
		echo -e "${COL_RED}Where ${#orphans[@]} packages seems no more used by any package:${NO_COLOR}"
		echo -e "$NO_COLOR${orphans[*]}$NO_COLOR"
	else
		echo -e "${COL_GREEN}Packages installed as dependencies but no more required:  ${COL_YELLOW}0"  
	fi
	echo -e "${COL_GREEN}Number of HoldPkg:  ${NO_COLOR}${COL_YELLOW}${#holdpkg[@]}"
	echo -e "${COL_GREEN}Number of IgnorePkg:  ${NO_COLOR}${COL_YELLOW}${#ignorepkg[@]}"
	echo -e "${COL_GREEN}Group ignored:  ${NO_COLOR}${COL_YELLOW}${ignoregroup[*]}"
	echo -e "\n${COL_BLUE}-----------------------------------------------${NO_COLOR}"	
}
...
showrepostats(){
	local NBCOLMAX=4
	local nbcol=1
	echo -e "${COL_GREEN}Number of configured repsitories:  ${NO_COLOR}${COL_YELLOW}${#repositories[@]}"
	echo -e "${COL_GREEN}Packages by repositories (ordered by pacman's priority)${NO_COLOR}:"
	local reponumber=0
	for repo in ${repositories[@]}; do
	...
	echo -e " ${NO_COLOR}others* ${COL_YELLOW}($(comm -2 -3 $tmp_files/installed $tmp_files/abs|wc -l))${NO_COLOR}"
	echo
	echo -e "${NO_COLOR}"*others" are packages not up to date or installed from local\nbuild or AUR Unsupported${NO_COLOR}"
	echo -e "\n${COL_BLUE}-----------------------------------------------${NO_COLOR}"	
}
...
showdiskusage()
{
	...
	for pkg in $PACMANROOT/local/*; do
		...
		size=$(($size+$sizeb))
		echo -ne "\r${COL_GREEN} Theorical space used by packages: ${COL_YELLOW}$(($size/1048576))Mo progression: $i/$countpkg\r" >&2
		i=$(($i+1))
	done
	echo -e "${COL_GREEN}Theorical space used by installed packages: ${COL_YELLOW}$(($size/1048576))Mo                               "
	...
	for pkg in $PACMANROOT/local/*; do
		...
		if [ -z $sizeb ]; then sizeb=0; fi
		size=$(($size+$sizeb))
		echo -ne "\r${COL_GREEN} Real space used by packages: ${COL_YELLOW}$(($size/1048576))Mo progression: $i/$countpkg\r" >&2
		i=$(($i+1))
	done
	echo -e "${COL_GREEN}Real space used by installed packages: ${COL_YELLOW}$(($size/1048576))Mo                               "
	# space used by download packages or sources in cache
	echo -e "${COL_GREEN}Space used by pkg downloaded in cache (cachedir): ${COL_YELLOW} $(du -skh $cachedir 2>/dev/null|awk '{print $1}')"
	if [ -z "$SRCDEST" ]; then
		srcdestsize="null"
	else
		srcdestsize=`du -skh $SRCDEST 2>/dev/null|awk '{print $1}'`
	fi
	echo -e "${COL_GREEN}Space used by src downloaded in cache: ${COL_YELLOW} $srcdestsize"
}
